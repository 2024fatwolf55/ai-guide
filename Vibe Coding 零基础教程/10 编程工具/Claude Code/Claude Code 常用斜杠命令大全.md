...
给你的项目配置做一次体检，也可以用 `/checkup` 这个别名。

它会自动扫描你的 Skills、CLAUDE.md 等配置文件，检查里面有没有写目录结构、依赖列表这些模型自己就能从代码库读到的冗余信息，找出没用到的 Skills 和 MCP 服务，检测本地和仓库里的 CLAUDE.md 有没有重复或矛盾，并建议把不需要每次都加载的内容迁移到可以按需加载的 Skills 里。

![](https://pic.yupi.icu/1/1785830100705-ba0f3f43-ba78-43e0-bf59-5dabcf2f2c61.png)

所有变更都会先给你看报告，等你确认后才会执行，不用担心它乱改配置。

这个命令是配合新模型时代的「提示词减法」思路推出的，因为官方发现规则写太多反而会抢占模型干活的注意力。想了解背后的原理，可以阅读本教程经验技巧板块「大厂 AI 编程经验」目录中的《Anthropic 官方 - 提示词精简方法》。
