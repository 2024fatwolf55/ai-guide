...
### CLI's New Role in the AI Era

You might be wondering: in this day and age, how did the oldest interaction style—the command line—suddenly become popular again?

In fact, the direction of major internet companies has changed recently. Google open-sourced Workspace CLI first, and then within just one week, Feishu, DingTalk, and WeCom all open-sourced their own CLI tools on GitHub one after another.

![](https://pic.yupi.icu/1/image-20260407142441568.png)

Why aren’t these big companies competing on prettier interfaces? Why are they all seemingly going backward?

The answer is just two letters: **AI**.

These companies aren’t building CLIs for humans—they’re building CLIs for AI.

**CLI is AI’s native language.** From the day large AI models were born, they’ve been learning massive amounts of code, command-line operations, and terminal output. Letting AI read one command and execute one operation is as natural as drinking water. On the other hand, asking AI to operate a graphical interface is much harder—it has to open a browser, wait for the page to load, find buttons, and deal with a bunch of irrelevant page elements and distractions along the way.

Someone even tested this: when AI was asked to complete real tasks through a browser, the success rate was only 35.8%; when the same tasks were completed through a CLI, the success rate was close to 100%!

So you’ll see a very interesting phenomenon. In the past, big companies built products for humans and tried their best to make the UI beautiful and easy to use. Now they’re returning to basics: **building products for AI and for AI to use—the simpler and more direct, the better.** Whoever CLI-ifies their product first gets connected by AI agents first, and gets to stay competitive in the AI era.

Overseas tech bloggers even wrote an article called *CLI is the new API*, which sparked a huge response. What it means is: in the past, products connected to each other through APIs; in the AI era, products connect with AI through CLIs.

At this point, you might ask: wasn’t MCP also very popular as a protocol connecting AI and tools? Why do we still need CLI?

With MCP, the names of all tools and their parameter formats have to be injected into the AI’s context. Once you have many tools, token consumption becomes outrageous. Some organizations have benchmarked this and found that for the same task, MCP’s token usage can be dozens of times that of CLI!

![](https://pic.yupi.icu/1/image-20260407123223554.png)

Also, MCP’s execution process is like a black box to humans, making problems hard to troubleshoot. CLI is different: if something goes wrong, you can copy the command directly into the terminal and run it yourself, and the error message is immediately clear.

Of course, this doesn’t mean MCP is outdated. In enterprise scenarios where unified permission control is required, MCP’s standardized authentication rules are still very valuable. And recently Cursor launched on-demand MCP loading, so instead of stuffing all tool definitions into context at once, it only loads a tool when the AI actually needs it, greatly reducing token consumption.
...
0) First, make sure your computer has a Node.js environment and the npm package installation tool. If not, just go to the [official Node.js website](https://nodejs.org/en/download) and download the beginner-friendly installer:
...
### Switching to Third-Party Models

Although Claude Code uses Anthropic’s Claude model by default, it does support switching to third-party models. Many Chinese large models (such as DeepSeek) provide APIs compatible with the OpenAI format, which can be connected directly to Claude Code.

I recommend using the open-source tool **CC Switch** to visually manage Claude Code’s model configuration. It comes with presets for more than 50 providers, so you can switch between different model vendors with one click, without manually editing configuration files yourself.
...
For example, to switch to DeepSeek V4, the steps are:

1. Install CC Switch (Mac users can install it through Homebrew)
2. Run the software and add DeepSeek as a provider
3. Fill in the API Key obtained from the DeepSeek Open Platform
4. Set the primary model to DeepSeek-V4-Pro (stronger agent capability and complex reasoning)
5. Save and enable it, then re-enter Claude Code to use it

![](https://pic.yupi.icu/1/1777343299981-c370c8ff-b24a-460a-891e-e08bc1efcf54-20260430231211935.png)

Through CC Switch, you can also switch freely among various Chinese models such as DeepSeek and Xiaomi MiMo, choosing the most suitable model as needed. For hands-on experience building a complete project with Claude Code + DeepSeek V4, you can read *Claude Code - AI Pelvic Floor Exercise Assistant Project Practice* in the “AI Creative Applications” category of this tutorial’s project practice section.
...
### CLI Open-Source Ecosystem

In addition to the AI programming tools above, the open-source CLI ecosystem is also evolving rapidly. Right now, CLIs from major companies like Feishu, DingTalk, WeCom, and Google basically all cover core business capabilities such as messaging, calendars, documents, and address books, and they all come with built-in AI Agent Skills that can be directly called by tools like Claude Code and Cursor.

![](https://pic.yupi.icu/1/image-20260407141854598.png)

Besides the official offerings from major companies, many interesting community projects are emerging too.

For example, [OpenCLI](https://github.com/jackwener/opencli) can turn any website, Electron app, or even local tool into a command-line interface. If you want AI to help you check Bilibili trending topics or Zhihu hot rankings, after installing OpenCLI you can get it done with a single command. It has dozens of built-in adapters covering platforms like Bilibili, Zhihu, Twitter, Reddit, and many more—it’s like installing a universal remote control for AI.

![](https://pic.yupi.icu/1/image-20260403143000224.png)

There’s also [CLI-Anything](https://github.com/HKUDS/CLI-Anything), which can automatically analyze the source code of an open-source software project, figure out the API logic behind each feature, and then automatically generate corresponding CLI commands. With it, almost any software can be rapidly turned into a CLI—perfect for AI to call.
...
7) Be more creative with prompts: For example, let Claude act as an interviewer—“Quiz me first to see whether I really understand these changes, and only open the PR if I pass.” Or say, “Based on everything you know now, throw this away and give me a more elegant solution.” You can also ask it to compare the differences between `main` and your feature branch, which often leads to better results.

8) Use subagents: If you want Claude to finish tasks faster, add a sentence like “use subagents” to your request, so independent subtasks can be delegated to subagents while the main agent’s context stays clean. You can also use Hooks to route permission requests to a more capable model, letting it judge whether an action is safe and approve it automatically.

9) Choose the right terminal environment: I recommend trying voice input too (on macOS you can use it by pressing `fn` twice), because speaking is about 3 times faster than typing. You’ll naturally describe requirements in more detail, and AI will understand you more accurately. You can also use `/statusline` to customize the status bar and display context usage and the current branch.

10) Learn new things through Claude: Enable an “explanatory” output style in the configuration so it explains the reasoning behind every change. You can also ask it to generate HTML slides to explain unfamiliar code, or draw ASCII diagrams to explain protocols and architecture—treat AI like your personal 1-on-1 tutor.

### 6. Build Your Own CLI Tool

We’ve introduced many ready-made CLI tools above, but in fact, you can absolutely build one yourself. If you already have your own product or tool, making a CLI lets users use it more conveniently through AI.

Here I especially want to introduce a gem of a technology for building CLIs: **React Ink**.

Some time ago, Claude Code’s source code was accidentally leaked. I dug through it and found that it was built with React Ink. Simply put, when we normally use React, it renders components into web pages in the browser. What React Ink does is render those same React components into terminal interfaces instead. This library already has tens of thousands of stars on GitHub, and well-known projects like Gatsby CLI and Prisma CLI use it too, so it’s very mature.

> Open-source link: https://github.com/vadimdemedes/ink

![](https://pic.yupi.icu/1/image-20260401124426974.png)

For example, by writing code like the following, you can render a simple terminal interface that displays a counter automatically increasing by 1 every second:

![](https://pic.yupi.icu/1/image-20260407144127215.png)

Now that we understand React Ink, let’s use it to build a CLI. Take my Programming Navigation product as an example—it's a learning and communication community for programmers. Once it’s turned into a CLI tool, users can search programming tutorials and view trending content directly in the terminal, and it also becomes convenient for AI agents to call.

The whole development process is actually pretty similar to building a web page. For a simple CLI tool, you can often just hand the whole task to AI in one go.

First, write a prompt for the AI:

```markdown
Help me use React Ink to develop a command-line tool named codefather-cli with the following features:

1) codefather search <keyword>
Fetch search results from Programming Navigation https://www.codefather.cn/search/all?searchText=<keyword>
Display the result list in the terminal, including title, author, and like count

2) codefather hot
Fetch the Programming Navigation trending list https://www.codefather.cn/hot/all_hot
Display the Top 20 trending items in the terminal, including ranking, title, author, and popularity

Requirement: support --help to view help information
```

Throw this prompt into Claude Code, Cursor, or another AI programming tool, and the AI can generate the complete project code for you.

![](https://pic.yupi.icu/1/image-20260407144535296.png)

The final running result looks roughly like this—not bad, right?

![](https://pic.yupi.icu/1/image-20260407150349886.png)

You can even try asking AI to use this tool. With `--help`, the AI can quickly understand how to use it and give an accurate answer—super fast!

![](https://pic.yupi.icu/1/image-20260407150632856.png)

That’s the charm of the CLI. For humans, it’s a highly practical productivity tool; for AI, it’s an even more natural operational interface.

### 7. Try Out Lark CLI Yourself

If you want to directly experience what a modern CLI feels like, try the recently open-sourced [Lark CLI](https://www.feishu.cn/feishu-cli) from Feishu. This tool lets you operate Feishu messages, calendars, documents, and more directly from the terminal.

![](https://pic.yupi.icu/1/image-20260407141759722.png)

First, install it with one command:
...
After installation, first configure the app information:
...
Open the link to configure your Feishu CLI app:

![](https://pic.yupi.icu/1/image-20260407125551163.png)

After the app is created successfully, you need to log in and authorize it. Choose which business capabilities you want to allow the CLI to operate:
...
Just follow the CLI guidance step by step:

![](https://pic.yupi.icu/1/image-20260407130308162.png)

During authorization, remember that you also need to approve the app in the Feishu admin console:

![](https://pic.yupi.icu/1/image-20260407130344599.png)

After the app is approved, you can run the login command again until you see “Authorization Successful”:

![](https://pic.yupi.icu/1/image-20260407130807612.png)

After that, you can operate Feishu through the command line.

For example, view today’s schedule:
...
Or even create a document directly:
...
In the past, you’d have to open the Feishu app and click around several times to complete these actions. Now one command gets it done.

There are so many CLI commands and parameters—what should you do if you forget how to use one during daily work? Just remember one universal rule: **if you don’t know, add `--help`**.
...
This example gives you an intuitive feel for the efficiency and convenience of CLIs, and helps you understand why so many major companies are now heavily investing in them.
...
If you want full video tutorials for mainstream tools like Cursor, Claude Code, and Codex—and want to use them to build multiple complete projects from scratch—check out Yupi’s video course [《AI Programming Practice for Absolute Beginners》](https://ai.codefather.cn/course/2087008226460565505). Every project is a full end-to-end hands-on build, from requirement analysis to deployment.
