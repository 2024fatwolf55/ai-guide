...
這不是 AI 變笨了，而是它的記憶出了問題。下面我就來教你如何透過 **上下文工程**，給 AI 裝上一個可靠的 “記憶補丁”。
...
想象一下，你在和一個新同事合作開發專案。如果你每次都要從頭解釋專案是什麼、用的什麼技術、有什麼規範，那效率會很低。但如果你們有一份共享的專案文件，新同事看一眼就能明白，那合作就會順暢很多。
...
AI 有一個上下文視窗（Context Window），可以理解為它的短期記憶容量。這個視窗是有限的，通常是幾千到幾十萬個 token（大約相當於幾千到幾十萬個單詞）。

當你和 AI 對話時，每條訊息都會佔用這個視窗的空間。對話越長，視窗就越滿。當視窗滿了，早期的對話內容就會被遺忘。

這就是為什麼 AI 會失憶，不是它真的忘了，而是早期的資訊已經被擠出視窗了。
...
不要在一個對話裡既討論登入功能，又討論支付功能，還討論效能最佳化。這樣會讓上下文變得混亂，AI 也容易搞混。

正確的做法是：

- 做登入功能時，開一個新對話
- 做完了，測試透過了，再開一個新對話做支付功能
- 遇到效能問題，又開一個新對話專門討論最佳化
...
- 使用函式式元件，不用 class 元件
- 所有元件都要有 TypeScript 型別
- 樣式使用 Tailwind CSS，不寫自定義 CSS
- API 呼叫統一使用 `/lib/api.ts` 中的函式
...
魚皮開源專案的 `README.md` 基本都是遵循這種結構化格式的，比如 [AI 零程式碼應用生成平臺專案](https://github.com/liyupi/yu-ai-code-mother)，供大家參考。

每次開始新對話時，把 `README.md` 的內容貼給 AI，它就能快速瞭解你的專案。
...
在新對話中，先提供完整的上下文，然後繼續工作。這樣雖然要重新開始，但能保證後續的對話質量。
...
Anthropic 把背後的原因總結成了一個概念，叫 **注意力預算**。模型的注意力總量是有限的，你給它的規則越多，它花在權衡這些規則上的注意力就越多，留給真正幹活的注意力就越少。所以「簡潔」不只是省 tokens，更是省注意力。想了解官方怎麼用這個思路把 Claude Code 的系統提示詞砍掉 80% 的，可以閱讀本板塊中的《Anthropic 官方 - 提示詞精簡方法》。
...
好的註釋能幫助 AI 理解程式碼的意圖，在修改時做出更合理的決策。
...
一個簡潔的線上筆記應用，支援 Markdown 編輯和實時儲存。
...
- 元件使用函式式元件 + Hooks
- 所有元件必須有 TypeScript 型別
- 樣式只用 Tailwind CSS
- API 呼叫統一使用 `/src/lib/api` 中的函式
- 狀態管理使用 Zustand
...
- 使用函式式元件
- 所有元件必須有 TypeScript 型別定義
- Props 型別命名：元件名 + Props (如 EditorProps)
- 樣式只用 Tailwind CSS
- 不使用 any 型別
- API 呼叫必須有錯誤處理
...
- 主色調：#6366F1
- 圓角：rounded-md (6px)
- 間距：使用 Tailwind 的標準間距 (4, 8, 12, 16...)
- 按鈕：px-4 py-2, hover 時變深色
- 輸入框：border-gray-300, focus 時 border-indigo-500

命名規範：
- 元件檔案：PascalCase (如 NoteEditor.tsx)
...
有了這個完整的上下文體系，無論你什麼時候開始新對話，只要把相關文件貼給 AI，它就能快速進入狀態，給出準確的答案。



## 寫在最後

上下文工程是 Vibe Coding 中最容易被忽視，但又極其重要的一環。很多人把精力都放在 “怎麼問” 上，卻忽略了 “怎麼讓 AI 記住”。

讓我總結一下本文的要點：

1. 上下文是 AI 的工作記憶：它決定了 AI 能否理解你的專案。

2. 管理好三個層次：專案級、功能級、對話級上下文都要照顧到。

3. 建立文件體系：README.md、TODO.md、上下文檔案缺一不可。

4. 利用工具特性：不同工具有不同的上下文機制，要善用它們。

5. 及時修復斷裂：發現 AI 失憶了，要立刻修復，不要繼續往下走。

6. 保持簡潔和更新：上下文要精煉、準確、及時更新。

掌握了上下文工程，你就能讓 AI 始終保持狀態，大大提高開發效率。

下一篇文章，我會講解除錯 AI 幻覺的方法，教你如何應對 AI 出錯的情況。
